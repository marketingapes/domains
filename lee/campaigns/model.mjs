import {brands,topics,VERSION} from './catalog.mjs';
...
export function createPlan(input={}){
 ...
 const mediaCents=tier==='major'?1000000:500000;
 ...
 return {schemaVersion:1,catalogVersion:VERSION,id:[brand.id,topic.id,input.start].join('-'),status:'draft',live:false,brand:brand.id,topic:topic.id,title:topic.name,firm:text(input.firm),start:input.start,end:schedule[13].date,geo:{location:text(input.location),radiusMiles:topic.id==='mva'?radius:null,verified:false},mediaCents,tier,managementMonthlyCents:250000,pricingStatus:'catalog_estimate_existing_terms_apply',variant:input.variant==='educational'?'educational':'direct',schedule,launchRequirements:['Confirm client agreement, scope and payment allocation','Confirm firm identity, geography, eligibility and disclosures','Approve copy and channel suitability','Verify inquiry destination and consent-aware follow-up','Test a complete inquiry through buyer receipt','Confirm measurement and budget authorization'],research:topic.source,qualificationReview:topic.review};
}
