...
(function () {
  ...
  /* ───── Public API ─────
     The stocked bootstrap owns window.EE (frozen, non-writable). This legacy helper registers itself under
     EE.legacy when the bootstrap is present, and only claims window.EE on a page with no bootstrap. */
  var legacyApi = {
  ...
  window.EETracking = legacyApi;
  if (window.EE && window.EE.__stocked && typeof window.EE.registerLegacy === 'function') {
    window.EE.registerLegacy(legacyApi);
  } else if (!window.EE) {
    window.EE = legacyApi;
  }
