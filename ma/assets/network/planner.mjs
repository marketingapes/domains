...
export function exportPlan(p) {
 return ['Marketing Apes','Your domain. A complete growth operation.','',p.title,p.industry+' | '+p.stage,'',p.summary,'','START HERE',p.firstStep,'','YOUR FIRST 30 DAYS',...p.weeks.map((s,i)=>'Week '+(i+1)+': '+s),'','MEASURE',...p.metrics.map(s=>'- '+s),'','SCOPE TO DISCUSS',...p.scope.map(s=>'- '+s),'','A guided starter plan, not a quote or guarantee. Scope, access, ad spend, usage, and timeline are agreed before work begins.','Discuss this plan: kyleg@marketingapes.com | 619-736-0356','https://marketingapes.com'].join('\n');
}
