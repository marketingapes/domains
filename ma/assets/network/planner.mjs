...
const plans = {
 response: {
  ...
  scope:['Domain + conversion page','Sofia conversation + human handoff','Follow-up + outcome reporting']
 },
 campaigns: {
  title:'Campaign + conversion operation',
  ...
  weeks:['Confirm the offer, audience, assets, ad-account access, budget, and conversion destination.','Build the domain landing page, campaign creative, capture, and follow-up path.','Verify test leads reach the right place and conversion reporting measures the agreed outcome.','Launch the approved campaign. Review lead quality and cost per outcome; change one variable at a time.'],
 ...
 operations: {
  title:'Workflow + domain operation',
  ...
  scope:['Purposeful domain + workflow','Connected tools + human approvals','Monitoring + ongoing iteration']
 }
};
...
export function exportPlan(p) {
 return ['Marketing Apes Network','Your domain. A complete growth operation.','',p.title,p.industry+' | '+p.stage,'',p.summary,'','START HERE',p.firstStep,'','YOUR FIRST 30 DAYS',...p.weeks.map((s,i)=>'Week '+(i+1)+': '+s),'','MEASURE',...p.metrics.map(s=>'- '+s),'','SCOPE TO DISCUSS',...p.scope.map(s=>'- '+s),'','A guided starter plan, not a quote or guarantee. Scope, access, ad spend, usage, and timeline are agreed before work begins.','Discuss this plan: kyleg@marketingapes.com | 619-736-0356','https://marketingapes.com'].join('\n');
}
